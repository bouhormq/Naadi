...
This Software Design Document (SDD) details the architecture and implementation of "Naadi," a platform that connects users to fitness studios via map-based browsing and enables businesses to manage their offerings. It ensures the system meets the needs of a user app (accessible via naadi.ma) and a business app (accessible via naadi.ma/business), leveraging Expo EAS Hosting and Firebase Firestore.
...
- **Naadi Business**: Enables studio owners to manage studios, classes, and bookings, available on mobile and naadi.ma/business (web).

Key features include:

- A web entry at naadi.ma with a "For Businesses" button linking to naadi.ma/business.
- A business signup form at naadi.ma/business collecting contact info.
...
- **Frontend**: Expo-based apps (naadi-user, naadi-business) for mobile and web, built with React Native and Expo Router.
...
```
+------------------+       +------------------+       +------------------+
| naadi.ma         |       | naadi.ma/business|       | EAS Hosting      |
| (naadi-user)     |       | (naadi-business) |       | (API + Web)      |
| - Home w/ Button |<----->| - Signup Form    |<----->| - API Routes     |
| - Map Browsing   |       | - Login          |       | - Web Deployment |
| - Login/Signup   |       | - Business Feat. |       |                  |
+------------------+       +------------------+       +------------------+
         |                            |                      |
         v                            v                      v
+------------------+       +------------------+       +------------------+
| Mobile App       |       | Mobile App       |       | Firebase         |
| (naadi-user)     |       | (naadi-business) |       | - Firestore      |
...
│   │   ├── businessSignup.ts # Business signup
│   │   └── login.ts       # Login
│   ├── bookings/
│   │   ├── create.ts
│   │   ├── cancel.ts
...
│       └── index.web.tsx
├── components/
│   ├── Header.tsx         # "For Businesses" button
...
#### 3.1.4 naadi-business

- **Purpose**: Business app for mobile and web (naadi.ma/business).
- **Structure**:
...
│   │       └── submit.ts
│   ├── _layout.tsx
│   ├── index.tsx          # naadi.ma/business home
│   ├── index.web.tsx
│   ├── login.tsx          # Redirects to business app
│   ├── login.web.tsx
│   ├── signup.tsx         # Business signup form
│   ├── signup.web.tsx
│   ├── (main)/
│   │   ├── _layout.tsx
│   │   ├── _layout.web.tsx
...
- **naadi.ma (naadi-user)**:
  - Home (app/index.tsx) includes a "For Businesses" button linking to naadi.ma/business.
  - Map browsing (app/(main)/studios/mapped.tsx) displays studios using location data.
  - Login (app/login.tsx) redirects to /(main) for user, or naadi.ma/business for business.
  - Signup (app/signup.tsx) creates a user account.
- **naadi.ma/business (naadi-business)**:
  ...
  - Login (app/login.tsx) redirects to /(main) for business, or naadi.ma for user.

### 4.2 API Endpoints

- **Hosted by EAS**:
  - `/api/auth/signup`: POST, user signup.
  - `/api/auth/businessSignup`: POST, business signup with contact info.
  - `/api/auth/login`: POST, authentication.
  - `/api/bookings/create`: POST, booking creation.
  - `/api/studios/stats`: GET, business stats.
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /studios/{studioId} {
      ...
      allow create, update, delete: if request.auth.token.firebase.sign_in_provider == 'custom' && isRole('business');
    }
    ...
    match /bookings/{bookingId} {
      allow read: if request.auth.uid == resource.data.userId || isRole('business');
      allow create, update: if request.auth.token.firebase.sign_in_provider == 'custom';
      allow delete: if false;
    }

    match /signupCodes/{codeId} {
      allow read, update: if request.auth.token.firebase.sign_in_provider == 'custom' && isRole('business');
...
- **naadi-business**: eas build (mobile), eas deploy --platform web (naadi.ma/business).
...
Naadi's design ensures a seamless experience for users browsing studios via a map and businesses managing their offerings, all within a lean monorepo with subrepos for shared logic. The structure supports naadi.ma and naadi.ma/business efficiently, meeting all requirements.