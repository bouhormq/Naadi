...
console.log('🔍 Testing Partner App Instructor API Endpoints...');

// Test get all instructors for a partner
async function testGetAllInstructorsEndpoint() {
      ...
      try {
        ...
        try {
          ...
          if (studioId) {
            ...
            // Get all instructors for this partner
            instructors = await API.getDocuments('instructors', { businessId: partner.id });
          }
  ...
  try {
    // Test case 1: Get all instructors for partner
    ...
    console.log('✅ Successfully retrieved all instructors for partner');
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            // Check if instructor belongs to this partner
            if (instructor.businessId !== partner.id) {
              return {
                status: 403,
                body: { error: 'Access denied. Instructor does not belong to this partner.' }
              };
            }
            ...
            for (const studioId of body.studioIds) {
              try {
                ...
                if (studio.businessId !== partner.id) {
                  return {
                    status: 403,
                    body: { error: `Studio ${studioId} does not belong to this partner` }
                  };
                }
              } catch (error) {
                return {
