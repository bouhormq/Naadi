...
console.log('🔍 Testing Partner App Class API Endpoints...');

// Test get all classes for a partner or studio
async function testGetAllClassesEndpoint() {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Get classes for this partner/studio
  ...
  try {
    // Test case 1: Get all classes for partner
    ...
    console.log('✅ Successfully retrieved all classes for partner');
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Verify the studio exists and belongs to this partner
          try {
            const studio = await API.getDocument('studios', body.studioId);
            if (studio.businessId !== partner.id) {
              return {
                status: 403,
                body: { error: 'Studio does not belong to this partner' }
              };
            }
          } catch (error) {
            return {
          ...
          // Check partner plan limits (simplified for testing)
          const classes = await API.getDocuments('classes', { businessId: partner.id });
          if (partner.plan !== 'premium' && classes.length >= 50) {
            return {
              status: 403,
          ...
          if (body.instructorId) {
            try {
              ...
              if (!instructor.businessId || instructor.businessId !== partner.id) {
                return {
                  status: 403,
                  body: { error: 'Instructor does not belong to this partner' }
                };
              }
            } catch (error) {
              return {
