...
import { subCategoriesData } from '@naadi/assets/data/venues'; // Assuming this is the path to your subcategories data
...
const SearchModal: React.FC<SearchModalProps> = ({
  ...
  const [activityQuery, setActivityQuery] = useState(searchCriteria?.activity || '');
  ...
  const [selectedDate, setSelectedDate] = useState(searchCriteria?.date || 'Any date');
  const [activeDateButton, setActiveDateButton] = useState('Any date');
  const [isTimePickerVisible, setTimePickerVisible] = useState(false);
  const [selectedTime, setSelectedTime] = useState(searchCriteria?.time || 'Any time');
  const [isLocationPickerVisible, setLocationPickerVisible] = useState(false);
  const [selectedLocation, setSelectedLocation] = useState(searchCriteria?.location || 'Current location');
  const [locationQuery, setLocationQuery] = useState('');

  // When modal opens, sync input fields with searchCriteria
  React.useEffect(() => {
    if (visible && searchCriteria) {
      setActivityQuery(searchCriteria.activity || '');
      setSelectedDate(searchCriteria.date || 'Any date');
      setSelectedTime(searchCriteria.time || 'Any time');
      setSelectedLocation(searchCriteria.location || 'Current location');
    }
        ...
        <View style={styles.modalContent}>
          ...
          <ScrollView>
            {isLocationPickerVisible ? (
              ...
              <View>
                <Text style={styles.headerTitle}>{selectedCategoryName || 'All Activities'}</Text>
                <View style={styles.inputContainer}>
                  <Ionicons
                    name="search-outline"
                    size={24}
                ...
                {/* If a category is selected, show only its activities. Otherwise, show all activities from all categories. */}
                {selectedCategoryName
                  ? subCategoriesData
                      .filter((cat) => cat.category === selectedCategoryName)
                      .map((cat) => {
                        const filteredSubs = cat.items.filter((activity) =>
                          activity.name.toLowerCase().includes(activityQuery.toLowerCase())
                        );
                        if (filteredSubs.length === 0) {
                          return (
                            <Text key="no-activities" style={{ color: 'gray', marginTop: 20 }}>
                              No activities found. You can search for a venue by name.
                            </Text>
                          );
                        }
                        return filteredSubs.map((activity) => (
                          <TouchableOpacity
                            key={activity.name}
                  ...
                  : (
                    <>
                      {/* All subcategory activities grouped by category */}
                      {subCategoriesData.map((cat) => (
                        <View key={cat.category}>
                          <TouchableOpacity
                            style={styles.activityRow}
                            onPress={() => {
                              setActivityQuery(cat.category);
                              setShowAllActivities(false);
                              setSelectedCategoryName(null);
                              // Immediately trigger search with this category
                              onSearch({
                          ...
                          {cat.items
                            ...
                            .map((activity) => (
                              <TouchableOpacity
                                key={activity.name}
                                style={styles.activityRowIndented}
                                onPress={() => {
                                  setActivityQuery(activity.name);
                                  setShowAllActivities(false);
                                  setSelectedCategoryName(null);
...
const styles = StyleSheet.create({
  ...
  categoryRow: {
    ...
    alignItems: 'center',
    marginBottom: 15,
    marginLeft: 0, // more left than subcategories
  },
  categoryName: {
    fontSize: 16,
    fontWeight: 'bold',
  ...
  activityRowIndented: {
    ...
    alignItems: 'center',
    marginBottom: 15,
    marginLeft: 15, // indented to the right
  },
});
