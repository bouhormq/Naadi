...
console.log('🔍 Testing Business App Studio API Endpoints...');
...
async function testGetAllStudiosEndpoint() {
  ...
  try {
    // Test case 1: Get all studios for business
    ...
    console.log('✅ Successfully retrieved all studios for business');
    ...
    // Test case 3: Unauthorized user (non-business user)
    ...
    console.log('✅ Correctly handled non-business user');
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            // Check if studio belongs to this business
...
async function testCreateStudioEndpoint() {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const existingStudios = await API.getDocuments('studios', { businessId: business.id });
          
          // Check if business subscription allows more studios
          const maxStudios = business.subscription?.maxStudios || 1;
  ...
  try {
    ...
    // Mock the API to return a business with maxStudios=0 to simulate reaching the limit
    const originalGetBusinessByOwnerId = API.getBusinessByOwnerId;
    
    API.getBusinessByOwnerId = async (ownerId) => {
      const business = await originalGetBusinessByOwnerId(ownerId);
      return {
        ...business,
        subscription: {
