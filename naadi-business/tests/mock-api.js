/**
 * Mock API Functions for Testing Business App
 * 
 * This file provides mock implementations of the @naadi/api functions
 * for use in testing the business app API endpoints.
 */
...
const getBusinessByOwnerId = async (ownerId) => {
  ...
  const business = Object.values(mockBusinesses).find(b => b.ownerId === ownerId);
  if (!business) throw new Error('Business not found for owner');
...
const getBusinessAnalytics = async (businessId) => {
  ...
  if (!analytics) throw new Error('Analytics not found for business');
