...
console.log('🔍 Testing Business App Classes API Endpoints...');
...
async function testListClassesEndpoint() {
  ...
  async function handleListClasses(request) {
    try {
      ...
      // Build filter
      const filter = { businessId: business.id };
...
async function testCreateClassEndpoint() {
  ...
  try {
    ...
    // Test case 3: Studio not owned by the business
    // First, create a studio owned by another business
    const otherStudio = {
      id: 'studio-other',
  ...
  async function handleUpdateClass(request) {
    try {
      ...
      try {
        ...
        if (classObj.businessId !== business.id) {
          return {
            status: 403,
        ...
        // If studio ID is being changed, verify the new studio belongs to this business
