...
export async function DELETE(request: ExpoRequest): Promise<ExpoResponse> {
  try {
    ...
    if (studio.businessId !== userId) {
      return new ExpoResponse(
        JSON.stringify({ error: 'You do not have permission to delete this studio' }),
        { status: 403, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    return new ExpoResponse(
      JSON.stringify({ success: true, message: 'Studio deleted successfully' }),
      { status: 200, headers: { 'Content-Type': 'application/json' } }
    );
  } catch (error: any) {
    console.error('Delete studio error:', error);
    return new ExpoResponse(
      JSON.stringify({ error: error.message || 'Failed to delete studio' }),
      { 
