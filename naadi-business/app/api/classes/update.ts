...
import { getUserIdFromToken, getUserById, getDocument, updateClass } from '@naadi/api';
...
export async function PUT(request: ExpoRequest): Promise<ExpoResponse> {
  try {
    ...
    const updateData = await request.json() as UpdateClassRequest;
    ...
    const classData = await getDocument<Class>('classes', updateData.classId);
    ...
    if (studio.businessId !== userId) {
      return new ExpoResponse(
        JSON.stringify({ error: 'You do not have permission to update classes for this studio' }),
        { status: 403, headers: { 'Content-Type': 'application/json' } }
      );
    }
    
    // Update the class
    const updatedClass = await updateClass(updateData, userId);
    ...
    return new ExpoResponse(
      JSON.stringify({ error: error.message || 'Failed to update class' }),
      { 
