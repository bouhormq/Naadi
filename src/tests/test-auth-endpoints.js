...
console.log('🔍 Testing Partner App Authentication API Endpoints...');

// Test partner signup endpoint
async function testBusinessSignupEndpoint() {
  console.log('\n📋 Testing POST /api/auth/partner-signup...');
  
  // Mock implementation of POST /api/auth/partner-signup endpoint
  async function handleBusinessSignup(request) {
    try {
      ...
      if (!body.email || !body.password || !body.businessName) {
        return {
          status: 400,
          body: { error: 'Email, password, and partner name are required' }
        };
      }
  ...
  try {
    // Test case 1: Successful partner signup
    const request1 = new MockExpoRequest({
      url: createRequestUrl('/api/auth/partner-signup'),
      method: 'POST',
      body: {
    ...
    if (!response1.body.user || !response1.body.partner || !response1.body.token || !response1.body.success) {
      throw new Error('Response should include user, partner, token, and success flag');
    }
    
    console.log('✅ Successfully tested partner signup with valid data');
    ...
    console.error('❌ Error testing partner signup endpoint:', error);
    return false;
  }
}
...
async function testLoginEndpoint() {
  ...
  async function handleLogin(request) {
    try {
      ...
      if (body.email === 'partner@example.com' && body.password === 'password123') {
        const user = API.mockData.users['user-123'];
        const partner = API.mockData.partneres['partner-123'];
...
// Test me endpoint (get current partner user)
async function testMeEndpoint() {
  ...
  async function handleMe(request) {
    try {
      ...
      try {
        ...
        let partner = null;
        try {
          partner = await API.getBusinessByOwnerId(userId);
        } catch (error) {
          // Partner not found, continue without partner details
        }
  ...
  try {
    ...
    if (!response1.body.user || !response1.body.partner) {
      throw new Error('Response should include user and partner data');
    }
    
    console.log('✅ Successfully retrieved partner user data with valid token');
